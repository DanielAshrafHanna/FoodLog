...
This replaces the public UI until you roll it back. New places, dishes, ratings, and **Mark as visited** writes go to production Supabase and stay there after the UI is restored.
...
### 19a. Per-user Bookmarks (table `restaurant_want_to_go`)

| | |
|--|--|
| **Behavior** | Approved editors (`state.canEdit`, same gate as Edit/Add place) can toggle a private **Bookmark** per restaurant from the selected restaurant's visible More menu. **Press and hold** a list row (or right-click on desktop) remains an optional shortcut to the same place action sheet. Restaurant rows do not repeat Bookmarks or Playlists buttons. When marked, a bookmark mark appears on list cards — **only for the signed-in user who marked it**. Signed-out and pending users see no sheet and no mark. The Places header **Bookmarks** chip filters to that personal set. User-facing copy is **Bookmarks**, not "Want to go", so it stays distinct from shared **Not visited** / **Visited** status. |
...
| | |
|--|--|
| **Behavior** | Each approved editor stores ONE rating (`0.5–5`) and review (`notes`) per dish in `dish_ratings`. Dish cards show the **average** score + review count, then up to **2 preview rows** (name stacked above stars; optional truncated review snippet). A visible **Add your review / Edit your review** action on every dish opens a review-only form attributed to the signed-in editor; the same action remains available in the full reviews sheet. Saving upserts only that editor's `dish_ratings` row and never changes dish metadata. Moving a review to Trash is explicit and recoverable. **Press and hold** a dish card with reviews (or right-click on desktop) remains an optional shortcut to the full review list. The dish edit form's **Your rating** / **Your review** fields remain compatible and also write only the current user's row. Dish metadata (name, photo, liked by) stays on the `dishes` row. The legacy `dishes.rating` / `dishes.notes` columns are no longer read or written by the app (left in place; backfilled into `dish_ratings` as `legacy@import`). |
...
| **Do not regress** | Reading/writing `dishes.rating` or `dishes.notes` for headline dish scores/reviews; letting a user write another user's dish rating row; hiding review creation behind dish metadata editing; rendering the full review list on the dish card (causes overlap/crowding); horizontal flex on `.dish-rating-row-head` that puts stars on the same line as names in narrow cards |
