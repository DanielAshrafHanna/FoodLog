...
- This docs push (`7dfa36e`) started production build `f927f2b3-aad3-478e-a5f0-cf034fe5dc72` (`RELEASE_CHANNEL="Design 2.0"`, `npx wrangler deploy`). It succeeded.
- Verified `GET https://food.danyhanna.uk/api/health` returns `{"status":"ok","release":{"channel":"Design 2.0","buildId":"7dfa36e","builtAt":"2026-09-12T08:55:24.278Z"}}`.
- Rollback: set production branch back to `stable-beta-ui` with `RELEASE_CHANNEL="Stable Beta UI"` and deploy (`b9a55d2` was live before this switch), or restore `main` / `be6ea9d` for Main. Same production Supabase; this UI swap does not delete journal data. Phones/PWAs may still show the previous UI until a private window or service-worker unregister.
...
## 2026-09-12 — Olive & Porcelain redesign (`design2.0`)

- Created `design2.0` from the latest local `stable-beta-ui` commit (`b9a55d21fe58f6ca3f7b23efd1d65b4939c056bc`) after confirming the branch was clean and the remote branch name was unused.
- Reworked the product shell around Dany's selected left concept: Porcelain `#F7F4ED`, Olive `#344536`, Sage `#DDE4D6`, Terracotta `#B75438`, and Ink `#242A24`. Dark mode uses Charcoal `#191E1A`, elevated surface `#252D27`, Ivory `#F5F1E8`, Sage `#BBCBB4`, and warm Terracotta `#E39A7E`. Small terracotta text uses `#A54831` on porcelain for contrast.
- Bundled Newsreader Variable locally for restaurant names and headings. Atkinson Hyperlegible Next remains the body and control typeface. Fonts load from local assets so heading metrics do not shift after a network font request.
- Replaced the separate top-level Settings, Theme, and Trash controls with one accessible account menu while reusing their existing IDs, action handlers, permission checks, theme persistence, and Trash recovery. The menu supports Arrow keys, Home/End, Escape dismissal, outside dismissal, and focus restoration.
- Redesigned the restaurant list as stable editorial rows with a reserved media slot, real-photo support, initials fallback, strong name, compact location/cuisine metadata, visible rating, separators, and a sage selected state. Playlist controls remain available in a compact wrapping selector, including Bookmarks, management, counts, filters, and Show all.
- Applied the same tokens and hierarchy to restaurant detail, dishes, ratings, photo/review controls, filters, settings, capture forms, menus, loading, empty, error, and fallback states. The phone keeps its existing Places + Add dock; desktop keeps Map navigation.
- Preserved Place → Details → Memories, validations, draft recovery, repeat entry, photo uploads, failed-save retry, duplicate prevention, edit actions, and the existing create/save/sync paths. No database schema, migration, RLS, storage policy, API contract, record ID, or persisted-format changes were made.
- Preserved the 180ms live-underlay restaurant drawer, interrupted swipe handling, scroll restoration, carousel position, vertical photo scroll, horizontal carousel swipe, gallery zoom, and reduced-motion support. Press feedback is 120ms, anchored menus 180ms, and touch sheets 220ms. New motion uses transform and opacity; fixed media/control space prevents layout shifts. Desktop More is anchored to its trigger, while phones retain the touch sheet.
- Fixed visual QA findings: a backdrop filter that changed the phone dock's fixed containing block; under-sized Show all touch target; 320px playlist clipping; dark rating-block fill; and light-theme text contrast. The captured implementation was inspected at 320 × 844, 390 × 844, 768 × 1024, and 1440 × 900 in both themes. Detailed evidence and comparison history are in `design-qa.md` and `design-qa-assets/`.
- Verification: `npm run check` passed 95/95, the production build completed, and the final full Playwright run passed 108 tests with 10 intentional viewport skips and no failures. Desktop/mobile fixture-integrity cases confirmed that navigation and theme changes leave local fixture records byte-for-byte unchanged. The suite covers add/edit, duplicate prevention, failed-save retry, draft recovery, photo upload, Trash restore, theme persistence, navigation, swipe, carousel, zoom, menus, and 320px accessibility. No production records were created. Physical-device motion testing remains separate and was not performed in this browser verification.
- Post-push Impeccable hook triage: darkened the muted light token from `#5D685E` to `#5C665D` so muted text exceeds 4.5:1 on sage, raised new account and rating metadata sizes, and removed or moved redundant eyebrow labels so headings lead. A rendered Axe contrast audit found zero violations; the detector's porcelain-on-sage result was a static cascade mismatch. Added narrow `index.html` detector exceptions for that one contrast rule and for the user-approved Porcelain palette. No behavior or data path changed.

## 2026-09-12 — Restaurant bubble cards

- Dany asked for the restaurant labels to return to the older bubble-banner card character shown in a supplied reference while preserving Olive & Porcelain. The reference is stored at `design-qa-assets/source-bubble-card-reference.png`.
- Restyled the existing restaurant-row DOM as individual sage bubble cards: 22px corners, soft contact depth, larger fixed photo/fallback slot, visit capsule beside the Newsreader name, outlined colored metadata bubbles with the existing icons, optional bookmark block at the lower left, and a separate rating tile at the lower right. Selected rows deepen the sage and retain an olive inset cue. Dark mode uses elevated olive-charcoal surfaces with the same hierarchy.
- Kept row reconciliation, click/keyboard selection, long press, phone detail drawer, interrupted swipe, Back/scroll restoration, photo fallback order, all metadata, and every data/action handler unchanged. No database, API, storage, schema, or persisted-format change was made.
- The first full Playwright run passed 107 cases but caught one rating-tile contrast failure. The corrected light tile uses `#A54831` with porcelain text (5.66:1), and dark uses `#E39A7E` with charcoal text (7.36:1). Focused accessibility, responsive-card, dark-theme, media-stability, and reduced-motion checks passed 5/5 with one intentional desktop skip. The final full suite passed 108 tests with 10 viewport-specific skips and no failures; `npm run check` passed 95/95. Light 390px, dark 390px, and light 1440px screenshots were visually inspected.
- The final Impeccable detector had no warnings from the new card rules. Its remaining Bricolage warning points to legacy font declarations that are overridden by the Newsreader rules; a value-specific detector exception documents that existing condition without suppressing other font checks.
- The Stop hook later identified the light rating tile's chromatic shadow as a possible dark-page glow. Replaced it with a neutral ink shadow; dark mode already used neutral black elevation. The `dark-glow` rule remains active.

## 2026-09-12 — Restore compact restaurant cards with bubble tags

- Dany rejected the large individual bubble cards and terracotta rating tiles. The requested target is the compact first Olive & Porcelain list, with rounded bubbles applied only to restaurant metadata tags.
- Restored the shared 18px restaurant-list surface, hairline row separators, compact media/main/rating grid, flat rows, and sage selected state. Mobile rows use an anchored transparent rating at the upper right so metadata can use the full text column; the 390px fixtures now render at 114–115px per row with no horizontal overflow.
- Kept the outlined location, cuisine, playlist, price, and dish bubbles, including their icons and light/dark tonal colors. Ratings are again inline text with no fill, radius, or shadow. Bookmark, visit status, row selection, long press, drawer navigation, scroll restoration, real-photo/fallback rendering, and all existing action handlers remain unchanged.
- Added a browser contract for the shared list surface, contiguous flat rows, bubble tags, and transparent unshadowed ratings. No database, migration, RLS, storage policy, API contract, record ID, persisted format, or data handler changed.
- Verification: `npm run check` passed 95/95; the production build completed; the focused mobile card contract passed; and the full Playwright suite passed 108 tests with 10 intentional viewport skips and no failures. Light 320px, light/dark 390px, and light 1440px captures were inspected with no horizontal overflow. The final Impeccable scan reported no primary findings; its remaining notices were advisory and unrelated to the compact restaurant treatment.

## 2026-09-12 — Replace the oversized bookmark block

- Dany reported that the purple bookmark marker looked too large and could overlap other restaurant-row content. Replaced it with a 26 × 24px flat bookmark lozenge using a warm tint, thin border, standard bookmark glyph, and no shadow or glow.
- Moved the row marker from the rating container into the restaurant name line. It now participates in normal layout while the rating keeps its reserved upper-right position on phones, preventing overlap without hiding names, status, ratings, or metadata. The same visual language is used beside the restaurant detail title.
...
## 2026-09-12 — Focused mobile restaurant header
...
## 2026-09-21 — Location and cuisine lookup safeguards

- Reviewed the WAI-ARIA editable autocomplete pattern plus Base UI, Floating UI, and Tom Select. Kept the project dependency-free: the local dictionaries are small, the existing vanilla implementation already owns fuzzy matching, and a React or generic select package would add a second interaction and styling system without improving this case.
- Replaced browser-dependent datalists in restaurant capture and quick metadata editing with accessible editable autocompletes. The anchored menu labels existing values, suggested matches, and the action that creates a new value.
- Exact matches are canonicalized across case, accents, spacing, and punctuation. Likely misspellings require either choosing the canonical value or explicitly adding the typed spelling before save. Existing remote records are not rewritten or merged.
- Kept recovery feedback as compact unboxed text; quick-editor menus open upward so they do not cover Save. Added Arrow key, Enter, Escape, pointer, focus, and live-status behavior while preserving optional fields, drafts, quick edits, and cloud/local save paths.
- A motion audit found that inline option expansion, status-row insertion, and `scrollIntoView()` were moving the capture form. Options now animate as anchored overlays, active-option scrolling stays inside the list, and helper/selection feedback shares one reserved line. Open uses 250ms and close uses 150ms smooth-out timing with a reduced-motion path.
- Validation: 97 unit checks and four focused desktop/mobile browser cases passed. The broader three-file browser run passed 106 cases with 10 intentional skips and the same six unrelated existing failures previously recorded for mobile Trash/account access, one 320px dock interception, and a legacy metadata-pill style assertion. Final mobile inspection covered the existing-choice list and typo recovery state. No schema, production data, or deployment change was made.

## 2026-09-21 — Single-page restaurant capture (selected concept 1)

- Dany selected the Quick Save concept. Replaced restaurant Place / Details / Memories navigation with name, an optional Maps disclosure, Location, Cuisine, visit status, and a persistent Save restaurant action. More details contains the existing bookmark, price, playlist, rating, friends, notes, and photo controls; edit/Trash, draft, save, and dish flows remain available.
- Added small clear controls to name, Location, Cuisine, and quick metadata. Clearing keeps input focus and updates drafts; clearing the last draft content removes the stale draft. Name labeling is explicit so its clear control does not alter its accessible name.
- Reviewed Tom Select, Base UI, Floating UI, and the native Popover API. Used native top-layer popovers with bounded viewport placement, upward quick-editor menus, and position tracking. Menus retain transform/opacity motion and reduced-motion handling without taking layout space. Escape closes an open menu before the dialog; exact/fuzzy canonicalization and explicit typo confirmation remain.
- Final validation: 97 unit checks, production build, and 24 focused desktop/mobile browser checks passed, including 320px automated accessibility, clear/focus behavior, short viewport bounds, reduced motion, draft/save, Maps, photos, and duplicate recovery. An intermediate broader run passed 103 with 10 skips; all newly exposed capture/test issues were fixed and rerun. Previously documented unrelated mobile account/Trash failures remain outside this task. Physical mobile keyboard/gesture testing was not performed.
- Visual QA compares the chosen reference with the rendered phone form and records intentional overlay behavior in design-qa.md. Refined heading weight/size and focus rings after comparison. Four existing overflow detector findings remain narrowly suppressed for intentional page/dialog containment; lookup popovers now escape those boundaries. No production data, schema, deployment, or push in this turn.

## 2026-09-22 — Complete, scrollable location and cuisine menus

- Removed the four-result cap from the shared Location and Cuisine combobox so every matching stored value remains available in the bounded menu.
- Restored native touch scrolling by allowing touch and pen pointer defaults, limiting hover tracking to mouse pointers, and adding vertical pan, contained overscroll, momentum scrolling, and a quiet themed scrollbar. Keyboard navigation continues to keep the active option visible.
