...
test("renaming a playlist updates the existing playlist instead of creating another", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Rename or delete playlist" }).click();
  const modal = page.locator("#playlistManageModal");
  await expect(modal).toBeVisible();
  await modal.getByLabel("Name").fill("Friday dinner");
  ...
  expect(saved.filter((place) => (place.playlists ?? []).includes("Friday dinner"))).toHaveLength(memberCount);
});
...
test("keeps Settings reachable and touch controls large enough on mobile", async ({ page }, testInfo) => {
  ...
  const mobileVisualContract = await page.evaluate(() => {
    ...
    return {
      ...
      rating: (() => {
        const rating = document.querySelector(".restaurant-row .rating-badge");
        return rating ? {
          fill: getComputedStyle(rating).backgroundColor,
          depth: getComputedStyle(rating).boxShadow,
          radius: getComputedStyle(rating).borderRadius
        } : null;
      })(),
  ...
  for (const surface of mobileVisualContract.rowSurfaces) {
    ...
    expect(Number.parseFloat(surface.radius)).toBeLessThanOrEqual(18);
  }
  ...
  expect(mobileVisualContract.rating?.fill).toBe("rgba(0, 0, 0, 0)");
  expect(mobileVisualContract.rating?.depth).toBe("none");
  expect(Number.parseFloat(mobileVisualContract.rating?.radius ?? "0")).toBeLessThanOrEqual(1);
