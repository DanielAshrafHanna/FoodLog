...
// Tabs and Continue may open a later step without a name. Save still validates in the form submit handlers.
export function createCaptureGuide({ form, body, steps, save, onStepChange, showNext = true, saveOnlyOnLast = false }) {
  if (saveOnlyOnLast) showNext = true;
  let current = 0;
  ...
  function go(index, focus = true) {
    ...
    if (next) {
      ...
      const nextIsPrimary = saveOnlyOnLast ? !last : (!first && !last);
      next.classList.toggle('primary-action', nextIsPrimary);
      next.classList.toggle('secondary-action', !nextIsPrimary);
    }
    if (saveOnlyOnLast) save.hidden = !last;
    const saveIsPrimary = saveOnlyOnLast ? last : (!next || first || last);
  ...
  return {
    ...
    showField(node) {
      ...
      if (i >= 0) go(i);
      const disclosure = node.closest('.capture-disclosure');
      if (disclosure) {
        disclosure.dataset.open = 'true';
        disclosure.querySelector(':scope > button[aria-expanded]')?.setAttribute('aria-expanded', 'true');
        const panel = disclosure.querySelector(':scope > .t-acc-panel');
