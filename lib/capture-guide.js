...
export function createCaptureGuide({ form, body, steps, save, validate, onStepChange }) {
  let current = 0;
  ...
  next.type = 'button'; next.className = 'secondary-action guide-next'; next.textContent = 'Continue';
  ...
  function go(index, focus = true) {
    ...
    const saveIsPrimary = first || last;
    ...
    body.scrollTop = 0;
    onStepChange?.({ index: current, first, last, save, next, back });
