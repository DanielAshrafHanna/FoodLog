...
export function isTransientNetworkError(error) {
  ...
  return ["failed to fetch", "networkerror", "network error", "load failed", "timed out", "timeout"]
    .some((fragment) => message.includes(fragment));
}

export async function retryTransient(task, { attempts = 3, wait = (ms) => new Promise((resolve) => setTimeout(resolve, ms)) } = {}) {
  let lastError;
