export function galleryPhotos(item) {
  ...
  if (item.photo && !(item.photos ?? []).some(photo => photo.photo === item.photo || (item.photoPath && photo.photoPath === item.photoPath))) {
    photos.unshift({ id: 'legacy', photo: item.photo, photoPath: item.photoPath, userId: /^[0-9a-f-]{36}\//i.test(item.photoPath ?? '') ? item.photoPath.split('/')[0] : '', contributorName: 'Contributor unknown' });
  }
