...
export function findSimilarLookupValues(candidate, values = [], { limit = 3 } = {}) {
  const normalizedCandidate = normalizeLookupValue(candidate);
  const compactCandidate = compactComparableText(normalizedCandidate);
  ...
  return values
    ...
    .map((value) => {
      ...
      const compactValue = compactComparableText(normalizedValue);
      const exact = compactCandidate === compactValue;
      const similarity = exact
        ? 1
        : Math.max(
            levenshteinSimilarity(compactCandidate, compactValue),
            bigramDiceSimilarity(compactCandidate, compactValue),
            tokenSimilarity(normalizedCandidate, normalizedValue)
          );
      const isSimilar = exact || (
        Math.min(compactCandidate.length, compactValue.length) >= 4 &&
        similarity >= 0.76
      );
      return { value, exact, similarity, isSimilar };
    })
...
export function formValidationCopy(field, fallbackMessage = "") {
  const fallback = String(fallbackMessage ?? "").trim();
  const native = String(field?.validationMessage ?? "").trim();
  if (field?.validity?.valueMissing) return fallback || native;
  return native || fallback;
}

export function mapPinStatusHint({
  placeCount = 0,
  pinCount = 0,
  unpinnedLinkCount = 0
} = {}) {
  const places = Math.max(0, Number(placeCount) || 0);
  const pins = Math.max(0, Number(pinCount) || 0);
  const unpinned = Math.max(0, Number(unpinnedLinkCount) || 0);
  if (pins > 0) {
    const missing = Math.max(0, places - pins);
    return `${pins} on map${missing > 0 ? ` · ${missing} without a parseable Maps URL` : ""}`;
  }
  if (unpinned > 0) {
    return unpinned === 1
      ? "1 place has a Maps link that is not pin-able yet. Open the place and paste a pin link, or open the current link."
      : `${unpinned} places have Maps links that are not pin-able yet. Open a place and paste a pin link, or open the current link.`;
  }
  return "No places with map coordinates yet. Add a Google Maps link when editing a restaurant.";
}
